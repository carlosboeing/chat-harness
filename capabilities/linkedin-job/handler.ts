...
function cleanHtmlFragment(value: string | undefined): string | null {
...
function observedJobIds(html: string): string[] {
...
export function parseLinkedInJobHtml(
  ...
  exact_job_id_match: boolean | null;
} {
...
function allowedLinkedInUrl(value: string): boolean {
  let url: URL;
  try {
    url = new URL(value);
  } catch {
...
async function fetchWithRestrictedRedirects(
  ...
  for (let redirects = 0; redirects <= 3; redirects += 1) {
    if (!allowedLinkedInUrl(current)) {
      throw new Error(
        "LinkedIn handler refused a redirect outside the allowlisted domain.",
      );
    }
...
export async function run(
  ...
  const signal = context.signal
    ? AbortSignal.any([context.signal, localTimeout])
    : localTimeout;

  let response: Response;
  try {
    response = await fetchWithRestrictedRedirects(guestUrl, signal);
  } catch (error) {
