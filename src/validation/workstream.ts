...
import { isIsoCalendarDate, parseMarkdownFrontmatter, FrontmatterError } from "./frontmatter.js";
import { parseMarkdownStructure } from "./markdown-structure.js";
import {
  formatSchemaErrors,
  validateWorkstreamFrontmatter,
} from "./schema.js";
...
function successorPath(
  workstreamsRoot: string,
  currentFile: string,
  raw: string,
): string | null {
  const resolved = path.resolve(path.dirname(currentFile), raw);
  const relative = path.relative(workstreamsRoot, resolved);
  if (
    relative === "" ||
    relative.startsWith("..") ||
...
export async function validateWorkstreamFile(
  file: string,
  options: WorkstreamValidationOptions,
): Promise<Finding[]> {
  ...
  try {
    ...
    findings.push(
      finding(
        ...
        error instanceof FrontmatterError ? error.message : "Unable to parse workstream frontmatter.",
        file,
      ),
    );
    return findings;
  ...
  for (const section of ["Objective", "Current state"] as const) {
  ...
  if (
    ...
    const successor = successorPath(
      options.workstreamsRoot,
      file,
      parsed.frontmatter.superseded_by,
    );
    if (successor === null) {
      findings.push(
        finding(
          "workstream.supersession_invalid",
          "superseded_by must resolve to a workstream inside .chat-harness/workstreams.",
          file,
        ),
      );
    } else {
      try {
        ...
        findings.push(
          finding(
            "workstream.supersession_invalid",
            "superseded_by target does not exist as a file.",
            file,
          ),
        );
      }
      ...
      if (!visited.has(successor)) {
        visited.add(file);
        try {
          const nested = await validateWorkstreamFile(successor, {
          ...
          if (nested.some((item) => item.severity === "error")) {
            findings.push(
              finding(
                "workstream.supersession_invalid",
                "superseded_by target does not satisfy the Workstream contract.",
                file,
              ),
            );
          }
        } catch {
          // Existence failure is already represented above.
        }
      }
    }
  }
